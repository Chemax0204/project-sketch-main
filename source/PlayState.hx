	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stageWhite': 
				...
				var bg:BGSprite = new BGSprite('bg', -600, -720, 0.9, 0.9);
				bg.antialiasing = ClientPrefs.globalAntialiasing;
				bg.scale.set(1.8, 1.8);
				bg.updateHitbox();
				add(bg);

				var floor:BGSprite = new BGSprite('floor', -595, -800, 0.9, 0.9);
				floor.antialiasing = ClientPrefs.globalAntialiasing;
				floor.scale.set(1.8, 1.8);
				floor.updateHitbox();
				add(floor);

				var pilars:BGSprite = new BGSprite('knux_pillars', -90, -810, 0.9, 0.9);
				pilars.antialiasing = ClientPrefs.globalAntialiasing;
				pilars.scale.set(1.2, 1.8);
				pilars.updateHitbox();
				add(pilars);

				var esmerald:BGSprite = new BGSprite('knux_emerald', -145, -520, 0.9, 0.9);
				esmerald.antialiasing = ClientPrefs.globalAntialiasing;
				esmerald.scale.set(1.2, 1.2);
				esmerald.updateHitbox();
				add(esmerald);

				var thing:BGSprite = new BGSprite('knux_stage', -145, -810, 0.9, 0.9);
				thing.antialiasing = ClientPrefs.globalAntialiasing;
				thing.scale.set(1.2, 1.8);
				thing.updateHitbox();
				add(thing);
			...
			case 'sketchHills': 
				...
				var floor:BGSprite = new BGSprite('sonic_flower', 110, 200, 0.9, 0.9);
				floor.antialiasing = ClientPrefs.globalAntialiasing;
				add(floor);

				var crab = new BGSprite('CrabmeatBGFreak', 620, 530, ['Crabmeat instance 1'], true);
				crab.antialiasing = ClientPrefs.globalAntialiasing;
				add(crab);

				var knuckles = new BGSprite('KnucksBG', 80, 405, ['Knuckles'], true);
				knuckles.antialiasing = ClientPrefs.globalAntialiasing;
				knuckles.scale.set(1.5, 1.5);
				knuckles.updateHitbox();
				add(knuckles);

				var tails = new BGSprite('TailsBG', -260, 430, ['Tails instance 1'], true);
				tails.antialiasing = ClientPrefs.globalAntialiasing;
				tails.scale.set(1.5, 1.5);
				tails.updateHitbox();
				add(tails);

				var amy = new BGSprite('AmyBG', 1280, 420, ['Amy'], true);
				amy.antialiasing = ClientPrefs.globalAntialiasing;
				amy.scale.set(1.5, 1.5);
				amy.updateHitbox();
				add(amy);
  ...
  public function clearShaderFromCamera(cam:String){
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong == 'Knucks'){
			switch (curBeat){
				case 200:
					camHUD.alpha = 0;
					FlxTween.tween(camHUD, {alpha: 1}, 5.8);
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['funky_fc', 'knucks_fc']);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					FlxG.sound.playMusic(Paths.music('sketchMenu'));
	...
	private function checkForAchievement(achievesToCheck:Array<String>):String {
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'funky_fc' | 'knucks_fc':
					if(songMisses < 1 && !changedDifficulty && !usedPractice)
					{
						switch(curSong)
						{
							case 'Funky Hill Zone':
								if(achievementName == 'funky_fc') unlock = true;
							case 'Knucks':
								if(achievementName == 'knucks_fc') unlock = true;
						}
					}
				}
